...
// Run manually: npx vitest run examples/__tests__/multi-turn-openai.test.ts (requires OPENAI_API_KEY)

import { it, expect } from 'vitest';
import { qaFn } from '../multi-turn-openai.js';

it('qaFn (OpenAI) continues conversation across turns via messages', async () => {
  // Turn 1
  ...
  expect(typeof turn1.output.followUp).toBe('string');
